...
namespace EnhancedBuildPanel
{
    ...
    public class MeshData : IComparable
    {
        ...
        {
            var localizedName = Locale.GetUnchecked("VEHICLE_TITLE", prefab.name);
        ...
        private static string GetSteamID(PrefabInfo prefab)
        {
            ...
            if (prefab.name.Contains("."))
            {
                ...
                if (!int.TryParse(steamID, out id)) return null;
            }

            return steamID;
        }

        private static void GetTriangleInfo(PrefabInfo prefab, out int triangles, out int lodTriangles, out float weight,
            out float lodWeight)
        {
            Mesh mesh = null;
            Mesh lodmesh = null;
            ...
            if (triangles != 0 && mesh.bounds != null)
            {
                ...
                var cubicMeters = boundsSize.x + boundsSize.y + boundsSize.z;
            ...
            {
                var boundsSize = lodmesh.bounds.size;
                var cubicMeters = boundsSize.x + boundsSize.y + boundsSize.z;
                if (Mathf.Round(cubicMeters) != 0)
